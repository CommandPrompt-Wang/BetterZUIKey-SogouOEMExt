...
- **完整的 …… 和 ——**：破折号/省略号直接打出两个
- **选区与分词修复**：部分宿主（Lexical / WebView 类输入框）把 `extend` 当 `move` 用，
  `Shift`/`Ctrl` + 方向键无法选字/选词 —— 开启后由模块接管并推进选区
  （`Shift`+←/→ 逐字、`Ctrl`+←/→ 按词、`Ctrl+Shift`+←/→ 按词选）
- **解除快捷键设置限制**：搜狗「外接键盘设置」默认不让录 `Alt`/`Shift`+字母的组合，开启后可录
  （顺带修掉"两个热键都清空却被报『与…快捷键冲突』"）
- **更好的软硬键盘切换**：本平台在物理键盘态下点输入框会弹出软键盘，而软键盘一出现物理键盘工具条
  就被销毁、物理键盘热键（含 `Alt`+字母）全部失效。开启后由模块接管切换：
  - 物理键盘态下点输入框**不再掉回软键盘**，软键盘态下点输入框正常弹出
  - 切到软键盘：`CSP`（搜狗里设的软硬切换热键）/ 工具栏最右的「软键盘」按钮 /
    2 秒内进-出-进同一个输入框 / 长按设置里的这一行
  - 切到物理键盘：直接用物理键盘打字，或长按这一行
...
> 若要使用“只响应系统框架语言切换消息”功能，**需要 BZK v1.7.0 以上**。
> 1.6.x 亦可联动部分功能，但无法使用轮转顺序自定义功能，系统框架只能切换最近使用的 2 门语言。
