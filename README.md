...
`[]()` 表示 `[`↔`]`、`(`↔`)`。因此**长度必须为偶数**，奇数不予保存。
留空 = 使用输入法默认匹配规则（此时 hook 不介入，完全走搜狗自己的表）。

**界面**：首页开关「引号/括号自动关闭」（默认开）；「**编辑匹配列表**」条目弹编辑窗口，
hint 与默认值均为建议串（18 对 / 36 字符）：

```
()[]{}（）【】《》""''“”‘’〈〉「」『』〖〗〔〕［］｛｝＜＞
```

输入框是**会自动换行的多行框**（36 字符单行会被截断）；多行框按回车能插换行，
所以**读文字时统一去掉 `\r\n`**，再做偶数校验 —— 于是"从别处粘一段带换行的表"也能直接存。

### 物理键盘补全（`physComplete`，功能 9）

搜狗在物理键盘上**不做**配对，所以这一侧**由模块自己注入**：
...
| 层 | 内容 | 默认 |
|---|---|---|
| 功能开关（UI） | **S「引号/括号自动关闭」** = 软键盘侧 | 开 |
| 功能开关（UI） | **9「物理键盘自动补全」** = 物理键盘侧 | 开 |
