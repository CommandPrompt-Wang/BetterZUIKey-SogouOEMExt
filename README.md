...
**开关4「智能编号」**：数字后面的 `。` / `）` 自动用半角 `.` / `)` —— 这样 `1.` `2)` 这类编号
才顺手。它属于**语义层**（在开关1/3 之后、开关2 之前），只作用于紧跟数字的那一下：
`1 .` → `1.`、`1 )` → `1)`、`1 ,` → `1，`（不动）、`abc .` → `abc。`、`1 . 2` → `1.2`。
判断依据是"上一次提交出去的最后一个字符是不是 ASCII 数字"。
...
**下拉「原样输出斜杠」**（关 / `/` / `\`）：因为 `/` 和 `\` 在搜狗里都变成 `、`，
所以给一个显式选择 —— 选 `/` 或 `\` 时，凡是搜狗交出的 `、` 都还原成该斜杠。
它属于**语义层**（在开关1/3 之后、开关2 之前），所以**遵循全半角**：
半角输出 `/`、全角输出 `／`。选「关」则保持搜狗原样（`、`）。
